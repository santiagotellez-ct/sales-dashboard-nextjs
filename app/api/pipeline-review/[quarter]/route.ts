...
import { computePipelineReviewData } from '@/lib/business-logic';
...
export async function GET(
  ...
  try {
    ...
    const data = computePipelineReviewData(quarter, weekFilter, deals, meetingEntries);
    return Response.json({ ...data, cache: getCacheInfo() });
  } catch (err) {
    console.error('[pipeline-review] Error:', err);
    return Response.json({ error: 'Failed to fetch pipeline review data' }, { status: 500 });
  }
}
